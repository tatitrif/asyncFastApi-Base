...
`git commit --amend -m "new message"` – сменить собщение к текущему коммиту
...
[Nginx](https://nginx.org/ru/) — это высокопроизоводительный веб-сервер. Мощный инструмент позволяющий конфигурировать веб-сервера любой сложности, справляется с большой нагрузкой и распределяет запросы по серверам. Количество настроек по балансировке огромное множество. С помощью nginx можно настроить HTTP2 для безопасности и кэширование для оптимизации. Кроме того, nginx позволяет перенаправлять запросы по разным приложениям системы (в случае, например, микросервисной архитектуры).
